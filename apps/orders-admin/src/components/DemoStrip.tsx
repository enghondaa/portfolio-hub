...
export function DemoStrip({
  ...
  /**
   * A sibling app that is part of the same demo. OrderFlow is two deployments
   * against one database, and the admin half was invisible: someone browsing
   * the shop had no way to learn an operator dashboard existed, let alone that
   * their order moving through it is the whole point.
   */
      ...
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-4 gap-y-2 px-5 py-2.5 sm:px-6">
        ...
        <div className="ml-auto flex items-center gap-3">
          {companion && (
            <a
              href={companion.url}
              className="rounded-full bg-[var(--color-accent)] px-2.5 py-0.5 font-mono text-[11px] text-[var(--color-neutral-50)] transition-opacity hover:opacity-85"
            >
              {companion.label}
            </a>
          )}
